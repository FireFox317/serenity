...
void init_stage2(void*)
{
    ...
    auto region = MUST(MM.allocate_kernel_region(8192, "Test KBuffer"sv, Memory::Region::Access::ReadWrite, AllocationStrategy::Reserve));
    dbgln("region: {}", region);
    u8* data = region->vaddr().as_ptr();
    for (u32 i = 0; i < (8192 / 4); i++) {
        ((u32*)data)[i] = i;
    }
    for (u32 i = 0; i < (8192 / 4); i++) {
        bool check = ((u32*)data)[i] == i;
        if (!check) {
            dbgln("whoops: {}: value: {}", i, ((u32*)data)[i]);
            VERIFY(false);
        }
    }

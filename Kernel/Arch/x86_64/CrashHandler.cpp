...
void handle_crash(Kernel::RegisterState const& regs, char const* description, int signal, bool out_of_memory)
{
    ...
    auto crashed_in_kernel = regs.previous_mode() == PreviousMode::KernelMode;
    ...
    dmesgln("CRASH: CPU #{} {} in {}", Processor::current_id(), description, regs.previous_mode() == PreviousMode::KernelMode ? "kernel"sv : "userspace"sv);
    dump_registers(regs);

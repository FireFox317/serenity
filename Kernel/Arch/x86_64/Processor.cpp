...
void Processor::enter_trap(TrapFrame& trap, bool raise_irq)
{
    ...
    if (current_thread) {
        ...
        if (current_thread->set_previous_mode(new_previous_mode) && trap.prev_irq_level == 0) {
            current_thread->update_time_scheduled(TimeManagement::scheduler_current_time(), new_previous_mode == PreviousMode::KernelMode, false);
        }
    } else {
...
void Processor::exit_trap(TrapFrame& trap)
{
    ...
    if (current_thread) {
        ...
        PreviousMode new_previous_mode;
        if (current_trap) {
            ...
            new_previous_mode = PreviousMode::KernelMode;
        }
